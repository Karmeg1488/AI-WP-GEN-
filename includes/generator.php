...
function aicg_generate_authors_categories_articles() {
    ...
    aicg_debug_log('Starting author generation process. Requested count: ' . $author_count);
    $created_authors = aicg_create_authors($author_count, $api_key);
    aicg_debug_log('Author generation result: ' . $created_authors . ' authors created');
    ...
    error_log('AI WP GEN - Found ' . count($authors) . ' author(s) to use for articles (requested ' . $author_count . ')');
    
    if (count($authors) === 0) {
        aicg_debug_log('ERROR: No generated authors found in database. Created authors: ' . $created_authors);
        aicg_debug_log('Attempting fallback: searching for any author user...');
        
        // Fallback: try to get any author
        $all_authors = get_users(['role' => 'author']);
        aicg_debug_log('Total authors with author role in database: ' . count($all_authors));
        
        if (count($all_authors) === 0) {
            aicg_debug_log('CRITICAL: No authors exist. Will need to create one or assign to admin.');
            // Try to get any user with capability to publish
            $admins = get_users(['role' => 'administrator', 'number' => 1]);
            if (count($admins) > 0) {
                $authors = $admins;
                aicg_debug_log('Using administrator as fallback: ' . $admins[0]->user_login);
            } else {
                return 'Failed to retrieve or create authors. No users with author or administrator role exist in the database.';
            }
        } else {
            $authors = $all_authors;
            aicg_debug_log('Using existing authors as fallback: ' . count($all_authors) . ' found');
        }
    }
    ...
    foreach ($categories as $category_name) {
        ...
        for ($i = 0; $i < $articles_per_category; $i++) {
            ...
            for ($attempt = 1; $attempt <= 3; $attempt++) {
                ...
                if ($content) {
                    aicg_debug_log('Article content generated on attempt ' . $attempt);
                    break;
                }
                aicg_debug_log('Article content generation failed on attempt ' . $attempt . ', retrying...');
                if ($attempt < 3) {
            ...
            if (!$content) {
                aicg_debug_log('Skipping article "' . $title . '" - failed to generate content after 3 attempts');
                continue;
            }

            // Log content length for debugging
            $content_length = strlen($content);
            aicg_debug_log('Article "' . $title . '" generated successfully. Content length: ' . $content_length . ' chars');
...
function aicg_create_authors($count, $api_key) {
    ...
    aicg_debug_log('Authors: need ' . $count . ', existing ' . count($existing_authors) . ', to create ' . $to_create);
    
    if ($to_create <= 0) {
        aicg_debug_log('Enough authors exist already');
        return 0; // Enough authors exist already
    }

    // Request OpenAI for real names
    $names = aicg_openai_generate_names($api_key, $to_create);
    aicg_debug_log('Received ' . count($names) . ' names from OpenAI: ' . json_encode($names));

    if (empty($names)) {
        aicg_debug_log('ERROR: No names returned from OpenAI');
        return 0;
    }

    foreach ($names as $full_name) {
        aicg_debug_log('Processing name: "' . $full_name . '"');
        
        $full_name = trim($full_name);
        if (empty($full_name)) {
            aicg_debug_log('Skipping empty name');
            continue;
        }
        
        $parts = explode(' ', $full_name);
        if (count($parts) < 2) {
            aicg_debug_log('Skipping name with less than 2 parts: ' . $full_name);
            continue;
        }
        ...
        if (!is_wp_error($user_id)) {
            ...
            $created++;
            aicg_debug_log('Author created: ' . $login . ' (ID: ' . $user_id . ')');
        } else {
            aicg_debug_log('Failed to create author: ' . $user_id->get_error_message());
        }
    }

    aicg_debug_log('Author creation completed. Total authors created: ' . $created . ' out of ' . $to_create);
    return $created;
}
...
function aicg_generate_contact_and_about_pages() {
    ...
    // Get main content/design prompt for customized page content
    $custom_prompt = get_option('ang_site_topic', '');
    $custom_context = !empty($custom_prompt) ? "Keep this in mind: \"$custom_prompt\". " : '';

    // Contact page prompt
    $contact_prompt = "Generate a comprehensive, detailed, and professional contact page content for a website called \"{$title}\" in {$language}. {$custom_context}The content should include:
...
Make it professional, thorough, engaging, and naturally structured with multiple paragraphs. Format as HTML without wrapper tags, only page body content. Minimum 600 words.";
    ...
    // About page prompt
    $about_prompt = "Generate a comprehensive, detailed, and engaging 'About Us' page content for a website called \"{$title}\" with the tagline \"{$tagline}\" in {$language}. {$custom_context}The content should include:
...
Make it professional, engaging, well-structured with varied paragraph lengths, and reflect the style of the tagline. Use natural language and specific details. Minimum 800 words. Format as HTML without wrapper tags.";
    ...
    if (empty($contact_content)) {
        aicg_debug_log('Failed to generate Contact page content');
        $contact_content = '<p>Unable to generate contact page. Please contact administrator.</p>';
    }
    if (empty($about_content)) {
        aicg_debug_log('Failed to generate About page content');
